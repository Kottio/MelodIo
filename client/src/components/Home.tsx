...
function Home() {
  ...
  async function getComposition() {
    ...
    if (response.ok) {
      ...
      setcompositions(result.compositions.reverse())
  ...
  function printComposition(composition: {}) {
    return <li key={composition?.id} className="flex items-center justify-center">
      <Link to={`/composition/${composition.id}`} className="group w-full flex items-center transition-all duration-300" >
        <div className="text-white">{getMusicStyleEmoji(composition.styles[0])}</div>
        <div className="p-4  border-b-1 border-b-zinc-400 flex justify-between items-center  w-full  group-hover:border-b-blue-300 transition-all duration-300" >
          <div >
            <div  >
              <span className="text-xl font-medium text-zinc-200 pr-3 group-hover:font-bold  group-hover:text-blue-300 transition-all duration-300">{composition.name}</span>
            ...
            <span className="text  pl-5 font-medium text-green-300" >{composition.styles[0]}</span>
          </div>
  ...
  return <>
    <div className='w-screen h-screen bg-[#101010] flex justify-center items-center'  >
      ...
      <section className={`bg-[#101010]  w-25/26 h-25/26 rounded flex flex-col  items-center ${!creating ? 'no-blur' : 'blur-sm'} transition-all duration-300`}>
        ...
        <div className="w-full pt-3 pb-2 flex justify-center items-center gap-3" >
          ...
          <div className="flex justify-end  w-9/12 pt-5 border-b-neutral-200 border-b-1"></div>
        ...
        <div className=" w-11/12   h-9/11 border-l-1  flex  mt-3 px-5 justify-between items-start ">
          ...
          <div className="flex justify-start items-start">
            <div className="flex flex-col  items-start gap-1 h-100 ">
              ...
              <button className="text-white flex items-center gap-2 hover:border-1 hover:px-3 px-1 py-2 mt-1  rounded transition-all duration-300"
          ...
          <div className="w-7/8 h-full mt-5 pl-5 flex flex-col items-center overflow-auto">
            {/* Conditional Rendering */}


            <ul className="flex flex-col gap-5 p-3 w-full overflow-auto ">
